...
class ClusterVis(Visualization):
    def __init__(self, data: np.ndarray, column_names: list[str], dash: Dash):
        ...
        self.normalized_data = MinMaxScaler().fit_transform(filtered_data)

        # PCA auf 2
        pca = PCA(n_components=2)
        self.reduced_data = pca.fit_transform(self.normalized_data)
    ...
    def build_score_plot(self) -> Graph:
        ...
        for k in k_values:
            ...
            silhouette_scores.append(silhouette_score(self.normalized_data, labels))
            davies_bouldin_scores.append(davies_bouldin_score(self.normalized_data, labels))
    ...
    def build_all_cluster_plots(self) -> list[Graph]:
        return [Graph(figure=scatter(
            ...
            color=KMeans(n_clusters=k).fit_predict(self.normalized_data),
            title=f'K-Means (k={k})',
